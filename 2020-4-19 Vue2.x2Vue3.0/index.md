...
1. props：`template`传递的参数，不像`vue2.x`可以通过`this.propsA`访问到`template`传递的参数，这里要通过`props.propsA`进行访问
   1. ctx：上下文，`setup`里面`this`不再指向`vue`实例，`ctx`有几个属性：`slots, root, parent, refs, attrs, listeners, isServer, ssrContext, emit`，其中`root`指向`vue实例，其他详细介绍可见[官方文档]( https://github.com/vuejs/composition-api/blob/master/README.zh-CN.md )
